...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/03/05 00:04:21 by luis-ffe          #+#    #+#             */
/*   Updated: 2024/04/05 16:22:27 by luis-ffe         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	create_list_quotes(char *cmd, t_lstexpand **in_cmd_list)
{
	...
	while (cmd[i] != '\0')
	{
		...
		{
			if (i > start)
				insert_lstexpand_node(in_cmd_list, \
				create_node_lstexpand(0, ft_strndup(cmd + start, i - start)));
			size = quotation_size(cmd, i);
			insert_lstexpand_node(in_cmd_list, \
			create_node_lstexpand(0, ft_strndup(cmd + i, size)));
			i = i + size;
			start = i;
		}
		else

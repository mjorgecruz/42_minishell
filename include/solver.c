...
/*   Updated: 2024/03/04 16:00:17 by masoares         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minishell.h"

int solver(t_token *cmd_list, t_info info)
{
	int		fd_in_out[2];
	int 	in_out[2];
	...
	while (cmd_list && cmd_list->cmds)
	{
		...
		if (fd_in_out[1] == STDOUT_FILENO)
			exec_correct_builtin(cmd_list->cmds, fd_in_out[0], in_out[0], info);
		else if (fd_in_out[1] > STDOUT_FILENO)
			cd_output_exec(cmd_list->cmds, fd_in_out, in_out, info);
		cmd_list = cmd_list->next;
	}
	return(1);
}

int	cd_output_exec(t_command *cmds, int *fd_in_out, int *in_out, t_info info)
{
	int		fd;
	...
	close(fd_in_out[1]);
	exec_correct_builtin(cmds, fd_in_out[0], in_out[0], info);
